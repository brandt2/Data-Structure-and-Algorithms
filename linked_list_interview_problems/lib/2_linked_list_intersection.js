...
function linkedListIntersection(list1, list2) {
  ...
  let obj = {};
  for (let current = list1.head; current !== null; current = current.next) {
    obj[current.value] = true;
  }

  for (let current = list2.head; current !== null; current = current.next) {
    if (obj[current.value] === true) return current;
  }
